---
title: "DialogV2 | Foundry Virtual Tabletop - API Documentation - Version 14"
url: "https://foundryvtt.com/api/v14/classes/foundry.applications.api.DialogV2.html"
category: "classes"
---
...
// `fd` will be null if the user canceled out of the dialog.  
if (!fd) {  
  console.error("User did not make a guess");  
  return;  
}  
const n = Math.ceil(CONFIG.Dice.randomUniform() * 10);  
// The `name` attributes of the dialog content become the keys of the returned `fd` variable.  
if ( n === fd.guess ) console.log("User guessed correctly.");  
...
The above examples all pass an HTML string for the `content` property of the dialog options; this is a good solution
when the text that needs to be rendered is simple, but is subject to HTML cleaning and difficult to maintain for
larger dialogs. The following two methods are more powerful and flexible ways of providing the necessary context.

1. Construct HTML elements with JS.

Using methods like `const content = document.createElement("div")`, you can provide an
[HTMLDivElement](https://developer.mozilla.org/en-US/docs/Web/API/HTMLDivElement) as the value of `content`.
The outermost element must be a plain div, then you can use methods like
[`content.insertAdjacentElement`](https://developer.mozilla.org/en-US/docs/Web/API/Element/insertAdjacentElement)
or [`content.insertAdjacentHTML`](https://developer.mozilla.org/en-US/docs/Web/API/Element/insertAdjacentHTML).
These methods are especially helpful when working with the form field helpers found in
[foundry.applications.fields](../modules/foundry.applications.fields.md), which return HTML elements.

The biggest advantage of this method is that the content is automatically trusted by Foundry and is not subject to
HTML cleaning. However, do keep in mind that the element will get stringified, so any listeners or other similar
adjustments will not carry forward to the dialog; you must still use the `render` option to attach listeners to the
dialog content.

2. Using [foundry.applications.handlebars.renderTemplate](../functions/foundry.applications.handlebars.renderTemplate.md)

For very complex dialogs, you can use `renderTemplate` to turn a handlebars template file into a string that can be
passed back to the `content`. If you directly pass the string, Foundry will clean the HTML, so you may still need to
use some of the above method to pass in a proper `HTMLDivElement` so Foundry can trust the generated HTML. Consider
also if you should instead be using a custom application instead of relying on a dialog.

#### Hierarchy ([View Summary](../hierarchy.md#foundry.applications.api.DialogV2))

* [ApplicationV2](foundry.applications.api.ApplicationV2.md)
  + DialogV2
    - [ShowToPlayersDialog](foundry.applications.sheets.journal.ShowToPlayersDialog.md)
    - [FolderExport](foundry.applications.sidebar.apps.FolderExport.md)
...
* close(options?: Partial<[ApplicationClosingOptions](../interfaces/foundry.applications.types.ApplicationClosingOptions.md)>): Promise<DialogV2>
...
* detachWindow(options?: [ApplicationRenderOptions](../interfaces/foundry.applications.types.ApplicationRenderOptions.md)): Promise<DialogV2>
