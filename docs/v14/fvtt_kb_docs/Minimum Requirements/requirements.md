...
url: "https://foundryvtt.com/article/requirements/"
...
* Relatively modern computer running Windows 10 or 11, macOS Sonoma (or newer), or Linux operating systems (Ubuntu 22+, Debian 11+, Redhat 9+, Arch) with support for 64-bit architecture.
  + Note: Older versions of Foundry VTT may work on previous versions of Windows, macOS, and Linux.
...
If your ISP provides an IPv6 address and your players can connect via IPv6 (confirmed via [this link](https://test-ipv6.com/)) there is a high possibility you do not need to set-up port forwarding, and your players will be able to connect to you via IPv6 instead, negating the need for Port-Forwarding.
...
* Ability to run Node 20+, (22 is required for V13 and below and 24 is required for V14 and above)
...
Many users repurpose a Raspberry Pi to host their Foundry VTT servers due to the lightweight nature of the Pi operating systems and the ease of reconfiguration. Provided the OS you are using supports Node 20+ and glibc 2.28 or higher, you should be able to run Foundry VTT without issue. Older models may still work, but are not officially supported. As a courtesy, we have provided a workaround below for guidance on how to rebuild our database engine for older, unsupported operating systems.
