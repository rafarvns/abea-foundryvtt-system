...
Can my premium content include AI generated content?

You can find the full details of what AI generated content can and can't be included in your module in our [AI Policy](https://foundryvtt.com/article/ai-policy/). The basics are this:

1. **Prepared content must be human-made.** Text, images, audio, and marketing materials that are intentionally pre-created must originate from human creative work. AI tools may provide limited assistance with editing or post-processing.
2. **Improvised content may be AI-generated.** Content generated at runtime in direct response to end-user prompts is permitted. We do not police what users choose to generate at their own table.
3. **Software code may be AI generated with specific conditions.** Package authors must personally understand and be prepared to maintain all code. Packages that generate software code at runtime must obtain user acknowledgement and consent for that code to be executable.
4. **Packages can be designated as "Zero AI".** Packages that exclusively contain user-facing content developed entirely without use of AI tools may designate that package as "Zero AI", advertising their work as 100% human created.
