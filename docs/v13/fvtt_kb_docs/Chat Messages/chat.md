---
title: "Chat Messages"
url: "https://foundryvtt.com/article/chat/"
...
Causes the message to be spoken by an associated character.
...
* The User has an Actor assigned to them in the User Configuration window, and
...
Whispers a message to the target. If the user sending the message does not have the "Whisper Private Messages" permission, they cannot use this chat command. If the whisper's target is an Actor that is assigned to a User as their character, the whisper will be sent to that User.
...
Rolls dice as a chat message. In the chat log, rolls can be clicked to expand their tooltip, showing the value of each individual roll. It can be clicked again to collapse the display. Refer to the [Basic Dice](/article/dice/ "Basic Dice") article for further details on how to roll dice in Foundry VTT. Once a roll is placed, you can expand the rolls to see detailed results by clicking on the rolled total.

### Chat Bubbles

![Chat Bubbles Example](https://r2.foundryvtt.com/website-uploads-public/screen/user_1/chat-bubbles-example-2020-03-28.jpg)

An example chat bubble displayed for an in-character message. The camera pans to focus on the speaker.

When an in-character or emote chat message is sent from the perspective of a placed Token, the camera will be moved to focus on the actor and a chat bubble will appear above that Token's head illustrating the dialog visually on the game canvas. There are two toggles found in core [Game Settings](/article/settings/ "Game Settings") which can configure the behavior of chat bubbles:
...
* The  [The ChatMessage Document](/api/classes/foundry.documents.ChatMessage.html " The ChatMessage Document")
* The [The Messages Collection](/api/classes/foundry.documents.collections.ChatMessages.html "The Messages Collection")
* The [The ChatLog Application](/api/classes/foundry.applications.sidebar.tabs.ChatLog.html "The ChatLog Application")